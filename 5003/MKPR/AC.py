...
def simulate_AC_with_queue(N, T, q_star, r, seed=42):
    np.random.seed(seed)
    ...
    for day in range(1, T):
        ...
        # 根据公式：p_k(t+1) = 1 - (q_k(t) + r) / [N(1 + r)]
        probabilities = 1 - (yesterday_counts + r) / ((1 + r) * N)
        probabilities = np.clip(probabilities, 0, None)
        if np.sum(probabilities) == 0:
            probabilities = np.ones(self.N) / self.N  # 均匀分配
...
for q_star in queue_lengths:
        ...
        for i, r in enumerate(r_values):
            q = simulate_AC_with_queue(N, M, q_star, r)
        ...
        plt.legend()
        plt.ylim(bottom=0)
        #plt.savefig(f"E:\\港科\\fall term\\5003\\final project\\fraction_distribution_ac(q={queue_length}).png")
        plt.savefig("AC-q512.png")
        plt.show()
    
        # 输出均值
        for r, mean in zip(r_values, mean_fractions):
            print(f'For r = {r}, mean fraction = {mean:.2f}')
